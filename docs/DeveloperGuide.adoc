...
With the JSON Object and the access_token taken from the user's configuration file, we create a HttpPost object and HttpClient Object, send it to LinkedIn, and analyze the response. In particular, if the response contains a updateUrl, it means that the post has been successfully posted, and we update the boolean value of postSuccess to true.
...
==== Design Considerations
===== Aspect: Allowing users to set privacy level
