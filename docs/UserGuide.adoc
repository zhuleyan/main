...
* In v 1.2: Able to add leads into CRM book and convert leads into contacts. Able to do some smart deduplication detection. Completion of edit command alias. Able to start process of LinkedIn Integration. Able to generate a color for the tags added.

* In v 1.3: Introduce calander for meetings. Duplicate detection based on phone number. Able to fully authenticate linkedIn, and link contacts to linkedIn. At least two alias commands will be completed.
...
* `list` +
`select 4` +
Selects the 4th person in the address book.
* `find Betsy` +
`select 1` +
